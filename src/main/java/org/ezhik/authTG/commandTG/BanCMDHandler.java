...
public class BanCMDHandler implements CommandHandler {
    ...
    public void execute(Update update) {
        ...
        if (user.isadmin || user.commands != null && user.commands.contains("ban")) {
            ...
            String message = "";
            String adminmsg = AuthTG.getMessage("bansuccess", "TG").replace("{PLAYER}", user1.playername);
            ...
            if (lettersCount > 2) {
                ...
                message = buildBanMessage(user.playername, reason, formattedDate, time);
                ...
                formattedDate = "0";
                message = buildBanMessage(user.playername, reason, "навсегда", time);
            ...
            if (user1.player != null) {
                Handler.kick(user1.playername, MessageHelper.legacySection(message));
            }

            AuthTG.loader.setBanTime(user1.uuid, formattedDate, reason, time, user.playername);
        } else {
            user.sendMessage(AuthTG.getMessage("bannoperm", "TG"));
        }
    }
