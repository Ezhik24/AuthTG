        ...
        if (args[1].equals("ban")) {
            if (list.get(0).equals("add")) {
                if (target.commands != null && !target.commands.contains("ban")) {
                    ...
                    user.sendMessage(AuthTG.getMessage("cmdsecondadd", "TG")
                            .replace("{PLAYER}", target.playername)
                            .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondban", "TG")));
                    if (target.activetg) {
                        target.sendMessage(AuthTG.getMessage("cmdsecondaddplayer", "TG")
                                .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondban", "TG")));
                    }
                    if (target.player != null) {
                        MessageHelper.send(target.player,
                                AuthTG.getMessage("cmdsecondaddplayer", "MC")
                                        .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondban", "MC")));
                    }
                } else {
                    user.sendMessage(AuthTG.getMessage("cmdsecondexcadd", "TG")
                            .replace("{PLAYER}", target.playername)
                            .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondban", "TG")));
                }
            }
            ...
            if (list.get(0).equals("rem")) {
                if (target.commands != null && target.commands.contains("ban")) {
                    ...
                    user.sendMessage(AuthTG.getMessage("cmdsecondrem", "TG")
                            .replace("{PLAYER}", target.playername)
                            .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondban", "TG")));
                    if (target.activetg) {
                        target.sendMessage(AuthTG.getMessage("cmdsecondremplayer", "TG")
                                .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondban", "TG")));
                    }
                    if (target.player != null) {
                        MessageHelper.send(target.player,
                                AuthTG.getMessage("cmdsecondremplayer", "MC")
                                        .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondban", "MC")));
                    }
                } else {
                    user.sendMessage(AuthTG.getMessage("cmdsecondexcrem", "TG")
                            .replace("{PLAYER}", target.playername)
                            .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondmute", "TG")));
                }
            }
        }
        ...
        if (args[1].equals("kick")) {
            if (list.get(0).equals("add")) {
                if (target.commands != null && !target.commands.contains("kick")) {
                    ...
                    user.sendMessage(AuthTG.getMessage("cmdsecondadd", "TG")
                            .replace("{PLAYER}", target.playername)
                            .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondkick", "TG")));
                    if (target.activetg) {
                        target.sendMessage(AuthTG.getMessage("cmdsecondaddplayer", "TG")
                                .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondkick", "TG")));
                    }
                    if (target.player != null) {
                        MessageHelper.send(target.player,
                                AuthTG.getMessage("cmdsecondaddplayer", "MC")
                                        .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondkick", "MC")));
                    }
                } else {
                    user.sendMessage(AuthTG.getMessage("cmdsecondexcadd", "TG")
                            .replace("{PLAYER}", target.playername)
            ...
            if (list.get(0).equals("add")) {
                if (target.commands != null && !target.commands.contains("mute")) {
                    ...
                    if (target.player != null) {
                        MessageHelper.send(target.player,
                                AuthTG.getMessage("cmdsecondaddplayer", "MC")
                                        .replace("{PERMISSION}", AuthTG.getMessage("cmdsecondmute", "MC")));
                    }
                } else {
                    user.sendMessage(AuthTG.getMessage("cmdsecondexcadd", "TG")
                            .replace("{PLAYER}", target.playername)
            ...
            if (args[1].equals(key)) {
                if (list.get(0).equals("add")) {
                    if (target.commands != null && !target.commands.contains(key)) {
                        ...
                        user.sendMessage(AuthTG.getMessage("cmdsecondadd", "TG")
                                .replace("{PLAYER}", target.playername)
                                .replace("{PERMISSION}", AuthTG.macro.getString(key + ".textbutton")));
                        if (target.activetg) {
                            target.sendMessage(AuthTG.getMessage("cmdsecondaddplayer", "TG")
                                    .replace("{PERMISSION}", AuthTG.macro.getString(key + ".textbutton")));
                        }
